...
import threading
import uuid
from battleships_pb2 import Attack, Request, Response, Status, Sunk
...
class BattleshipClient(ClientInterface):
    ...
    def sunk(self, ship):
        """
        This method indicates a ship has been sunk and which ship it is
        """
        if ship is None or type(ship) is not str:
            raise ValueError('Parameter ship must be a string!')
        self.__send(Request(which_ship=Sunk(sunk_ship=ship)))
